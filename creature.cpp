...
int32_t Creature::getWalkDelay(Direction dir) const
{
	...
	int64_t stepDuration = getStepDuration(dir) * lastStepCost;
...
void Creature::onThink(uint32_t interval)
{
	...
	if (followCreature) {
		...
		if (forceUpdateFollowPath || walkUpdateTicks >= 1000) {
			walkUpdateTicks = 0;
...
int64_t Creature::getStepDuration() const
{
	...
	if (monster && monster->isTargetNearby() && monster->mType->info.targetDistance > 1 && !monster->isFleeing() && !monster->getMaster()) {
		// Logica para quando encostar no monstros de distancia ele anda devagar
		// Antes um erro o monstro após correr e voltar a lutar depois da curar pecorria devagar.
		if (!monster->canHitBack()) {
			stepDuration *= 2;
		}
	}
