	...
	const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		uint32_t charges = std::max(static_cast<uint32_t>(1), static_cast<uint32_t>(item == nullptr ? it.charges : item->getItemCount()));
...
std::string Item::getWeightDescription(const ItemType& it, uint32_t weight, uint32_t count /*= 1*/)
{
	...
	if (it.luck != 0) {
		ss << "Luck loot +" << float(it.luck * 0.1) << "%.\n";
	}
	if (it.luckGold != 0) {
		ss << "Luck gold +" << float(it.luckGold * 0.1) << "%.\n";
	}
	if (it.luckJewel != 0) {
		ss << "Luck jewel +" << float(it.luckJewel * 0.1) << "%.\n";
	}
	if (it.luckRune!= 0) {
		ss << "Luck rune +" << float(it.luckRune * 0.1) << "%.\n";
	}
	if (it.luckCharge != 0) {
		ss << "Luck ammo +" << float(it.luckCharge * 0.1) << "%.\n";
	}
