...
bool Spell::playerRuneSpellCheck(Player* player, const Position& toPos)
{
	...
	if (aggressive && topVisibleCreature && player->hasSecureMode()) {
...
bool ConjureSpell::conjureItem(Creature* creature) const
{
	...
	uint32_t conjureCost = getManaCost(player);
	const uint32_t realManaCost = conjureCost;
	uint32_t conjureCountModific = conjureCount;
	Luck luck = player->getLuck();
	bool makeRune = false, makeAmmo = false;

	if (luck.luckRune > 0 && Item::items[conjureId].isRune()) {
		int32_t makeChance = 0;
		makeChance = std::ceil(luck.luckRune * 0.1);

		if (makeChance > 100) makeChance = 100;
		if (uniform_random(0, 100) <= makeChance) {
			conjureCountModific *= 2;
			conjureCost = uniform_random(conjureCost / 2, conjureCost);
			makeRune = true;
		}
	}
	else if (luck.luckCharge > 0 && Item::items[conjureId].ammoType) {
		int32_t makeChance = 0;
		makeChance = std::ceil(luck.luckCharge * 0.1);
	...
	if ((makeAmmo || makeRune) && conjureCost > std::ceil(realManaCost * 0.9)) {
		conjureCountModific = uniform_random(conjureCount + 1, conjureCountModific);
	}
	else {
		conjureCountModific = uniform_random(conjureCount, conjureCountModific);
	}

	// Make rune in backpack
	if ((g_config.getBoolean(ConfigManager::MAKE_RUNE_BACKPACK_FREE) && !player->isPremium()) || (g_config.getBoolean(ConfigManager::MAKE_RUNE_BACKPACK_PREMIUM) && player->isPremium())) {
		...
		if (reagentId != 0) {
			...
			Item* newItem = Item::CreateItem(conjureId, conjureCountModific);
			if (!newItem) {
				return false;
			}

			Item* ret = g_game.transformItem(item, conjureId, conjureCountModific);
			if (!ret) {
			...
			Spell::postCastSpell(player, conjureCost, soul);
		}
		else {
			Item* newItem = Item::CreateItem(conjureId, conjureCountModific);
			if (!newItem) {
		...
		if (makeRune) {
			std::ostringstream ss;
			ss << "Your luck created " << conjureCountModific << "x " << Item::items[conjureId].name << " with " << conjureCost << " of mana.";
			player->sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, ss.str());
		}
		else if (makeAmmo) {
			std::ostringstream ss;
			ss << "Your luck created " << conjureCountModific << "x " << Item::items[conjureId].name << " with " << conjureCost << " of mana";
		...
		if (reagentId != 0) {
			...
			if (item && item->getID() == reagentId && player->getMana() >= realManaCost) {
				foundReagent = true;
			...
			g_game.startDecay(newItem);
			Spell::postCastSpell(player, conjureCost);
		}

		if (makeRune) {
			std::ostringstream ss;
			ss << "Your luck created " << conjureCountModific << " rune with " << conjureCost << " of mana.";
			player->sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, ss.str());
		}
		else if (makeAmmo) {
			std::ostringstream ss;
			ss << "Your luck created " << conjureCountModific << " ammo with " << conjureCost << " of mana";
			player->sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, ss.str());
		}
		postCastSpell(player, true, false);
		g_game.addMagicEffect(player->getPosition(), CONST_ME_MAGIC_RED);
	...
	bool RuneSpell::executeUse(Player * player, Item * item, const Position&, Thing * target, const Position & toPosition)
	{
		...
		if (hasCharges && item && g_config.getBoolean(ConfigManager::REMOVE_RUNE_CHARGES)) {
			...
			Luck luck = player->getLuck();
			int32_t chance = std::ceil(luck.luckRune * 0.1);

			if (luck.luckRune > 0 && item->getCharges() > 0 && uniform_random(0, 100) <= chance) {
				std::ostringstream ss;
				ss << "Your luck saved the " << item->getName() << " charge.";
				player->sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, ss.str());
				return true;
			}
