...
bool ConditionDamage::setParam(ConditionParam_t param, int32_t value)
{
	switch (param) {
		...
		case CONDITION_PARAM_EFFECT:
			condition_effect = value;
			return true;

		case CONDITION_PARAM_SPECIAL:
			condition_special = value;
			return true;
...
bool ConditionDamage::executeCondition(Creature* creature, int32_t)
{
	if (conditionType == CONDITION_FIRE) {
		...
		const int32_t special = condition_special;

		if (r_cycle && special == 0) {
			if (count <= 0) {
				count = max_count;
		...
		else if (r_cycle && special != 0) {
			if (count <= 0) {
				count = max_count;
				cycle = r_cycle + 2 * (r_cycle <= 0) - 1;
				if (Player* player = creature->getPlayer()) {
					doDamage(creature, (-max_value / 2));
				}
				else {
					doDamage(creature, -max_value);
				}
				//g_game.addMagicEffect(creature->getPosition(), condition_effect);
			}
			else {
				--count;
			}
