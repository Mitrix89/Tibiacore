...
class ConditionDamage final : public Condition
{
	public:
		...
		ConditionDamage(ConditionId_t id, ConditionType_t type, uint32_t subId = 0) :
			...
			else if (type == CONDITION_ICE) {
				count = max_count = 10;
			}
			else if (type == CONDITION_HOLY) {
				count = max_count = 10;
			}
			else if (type == CONDITION_DEATH) {
				count = max_count = 10;
	...
	protected:
		...
		int32_t max_value = 0;
		int32_t condition_effect = 0;
		int32_t hit_damage_2 = 0;
		int32_t condition_special = 0;
