...
void MonsterType::createLoot(Container* corpse)
{
	...
	for (auto it = info.lootItems.rbegin(), end = info.lootItems.rend(); it != end; ++it) {
		std::vector<Item*> itemList = createLootItem(*it, corpse);
		if (itemList.empty()) {
			continue;
		}
	...
	if (g_config.getBoolean(ConfigManager::SHOW_MONSTER_LOOT)) {
		...
		if (owner && owner->isPremium()) {
			...
			if (owner->getParty()) {
				...
				owner->sendChannelMessage("", ss.str(), TALKTYPE_CHANNEL_O, 21);
			}
		}
	}
...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, Container* corpse)
{
	const uint32_t rate = g_config.getNumber(ConfigManager::RATE_LOOT);
	uint32_t itemCount = 0;
	uint32_t randvalue = Monsters::getLootRandom();
	uint32_t countMax = lootBlock.countmax + 1;

	const uint32_t chance = lootBlock.chance;

	Player* owner = g_game.getPlayerByID(corpse->getCorpseOwner());
	bool lucklootEq = false;
	uint32_t luckloot = 0;
	uint32_t gold = 0;
	uint32_t jewel = 0;
	...
	if (owner) {
		Luck luck = owner->getLuck();
		if (luck.luck > 0) {
			if (chance < 10 && luck.luck < 100) {
				luckloot = float(chance * 0.01 * luck.luck / 10);
			}
			else {
				luckloot = std::ceil(chance * 0.01 * luck.luck / 10);
			}
			luckloot += chance;
			lucklootEq = true;
		}
		...
		if (luck.luckGold > 0) {
			gold = std::ceil((chance * 0.01 * luck.luckGold) / 10);
			lucklootEq ? gold += round(luckloot * 0.01) : gold;
		}

		if (luck.luckJewel > 0) {
			jewel = std::ceil(chance * 0.01 * luck.luckJewel / 10);
			jewel += chance;
			lucklootEq ? jewel += round(luckloot * 0.01) : jewel;
		}

		if (randvalue < rate * luckloot) {
		...
		if (luck.luckGold > 0 && randvalue < rate * gold && lootBlock.id == 3031) {
			countMax = std::ceil(uniform_random(countMax + 1, (countMax - 1) * 2));
			itemCount = countMax;
			ss << "Your luck golds";
		}
		if (luck.luckJewel > 0 && randvalue < rate * jewel && (lootBlock.id >= 3020 && lootBlock.id <= 3030 || lootBlock.id > 3031 && lootBlock.id <= 3034 || lootBlock.id > 3035 && lootBlock.id <= 3042)) {
			if (Item::items[lootBlock.id].stackable) {
				countMax = std::ceil(uniform_random(countMax - 1, (countMax - 1) * 2));
				itemCount = countMax;
			}
		...
		if ((luck.luckGold > 0 || luck.luckJewel > 0) && itemCount > 0 && (itemCount > lootBlock.countmax)) {
			itemCount > 1 ? ss << " yields " << itemCount << "x " << Item::items[lootBlock.id].pluralName << "." :
				ss << " yields " << itemCount << "x " << Item::items[lootBlock.id].name << ".";
			owner->sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, ss.str());
		}

		if (chance < 5 && itemCount > 0 && lucklootEq) {
			std::ostringstream sss;
			itemCount > 1 ? sss << "Lucky: " << owner->getName() << " dropped very rare " << itemCount << "x " << Item::items[lootBlock.id].pluralName << " from " << nameDescription << "." :
				sss << "Lucky: " << owner->getName() << " dropped very rare " << itemCount << "x " << Item::items[lootBlock.id].name << " from " << nameDescription << ".";
			for (const auto& it : g_game.getPlayers()) {
				it.second->sendTextMessage(MESSAGE_STATUS_CONSOLE_LBLUE, sss.str());
			}
		}
	}
	else {
		if (randvalue < rate * chance) {
			if (Item::items[lootBlock.id].stackable) {
				itemCount = randvalue % countMax;
			}
			else {
...
{
	auto it = lootblock.childLoot.begin();
	auto end = lootblock.childLoot.end(); // Poprawka w deklaracji koñca

	for (; it != end && parent->size() < parent->capacity(); ++it) {
		auto itemList = createLootItem(*it, parent);
	...
	if (isScripted) {
		...
		if (tmpName == "physical") {
			...
			combat->setParam(COMBAT_PARAM_CREATEITEM, ITEM_ENERGYFIELD_PVP);
			       ...
			       tmpName == "deathcondition" || tmpName == "holycondition" ||
			...
			else if (tmpName == "holycondition") {
				conditionType = CONDITION_HOLY;
			}
			else if (tmpName == "deathycondition") {
				conditionType = CONDITION_DEATH;
			}
			else if (tmpName == "icecondition") {
				conditionType = CONDITION_ICE;
			}
			...
			else if (conditionType == CONDITION_ICE) {
				count = 15;
				cycle /= 20;
			}
			else if (conditionType == CONDITION_HOLY) {
				count = 20;
				cycle /= 20;
			}
			else if (conditionType == CONDITION_DEATH) {
				count = 35;
				cycle /= 20;
			}
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, std::list<std::pair<MonsterType*, std::string>>& monsterScriptList, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("extraMonster"))) {
		mType->extraMonster = attr.as_string();
	}
	if ((attr = monsterNode.attribute("extraChance"))) {
		mType->extraChance = pugi::cast<uint32_t>(attr.value());
	}
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			} else if (strcasecmp(attrName, "hitback") == 0) {
				mType->info.canHitBack = attr.as_bool(); 
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					mType->info.conditionImmunities |= CONDITION_DEATH;
				}
				else if (tmpStrValue == "ice") {
					mType->info.damageImmunities |= COMBAT_ICEDAMAGE;
					mType->info.conditionImmunities |= CONDITION_ICE;
				}
				else if (tmpStrValue == "holy") {
					mType->info.damageImmunities |= COMBAT_HOLYDAMAGE;
					mType->info.conditionImmunities |= CONDITION_HOLY;
