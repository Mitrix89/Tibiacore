...
void LuaScriptInterface::registerFunctions()
{
	...
	lua_register(luaState, "setOnSpawn", LuaScriptInterface::luaSetOnSpawn);
	...
	registerEnum(CONST_ME_HOLYATTACK)
	registerEnum(CONST_ME_DEATHATTACK)
	registerEnum(CONST_ME_FIRESTRIKE)
	...
	registerEnum(CONST_ANI_ICEATTACK)
	registerEnum(CONST_ANI_HOLYATTACK)
	registerEnum(CONST_ANI_DEATHATTACK)
	registerEnum(CONST_ANI_FIRESTRIKE)
	...
	registerEnumIn("configKeys", ConfigManager::RESTORE_UPLEVEL_FREE)
	registerEnumIn("configKeys", ConfigManager::RESTORE_UPLEVEL_PREMIUM)
	registerEnumIn("configKeys", ConfigManager::DROP_SPEAR_FREE)
	registerEnumIn("configKeys", ConfigManager::DROP_SPEAR_PREMIUM)
	registerEnumIn("configKeys", ConfigManager::MAKE_RUNE_BACKPACK_FREE)
	registerEnumIn("configKeys", ConfigManager::MAKE_RUNE_BACKPACK_PREMIUM)
	...
	registerMethod("ItemType", "getLuck", LuaScriptInterface::luaItemTypeGetLuck);
	...
	registerMethod("MonsterType", "canHitBack", LuaScriptInterface::luaMonsterTypeCanHitBack);
...
int LuaScriptInterface::luaSetOnSpawn(lua_State* L)
{
...
int LuaScriptInterface::luaItemTypeGetLuck(lua_State* L)
{
	// itemType:getLuck()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, itemType->luck);
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaConditionSetTiming(lua_State* L)
{
	...
	if (condition) {
		...
		else if (condition->getType() == CONDITION_ICE) {
			condition->setParam(CONDITION_PARAM_COUNT, 10);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 10);

			count /= 20;
		}
		else if (condition->getType() == CONDITION_HOLY) {
			condition->setParam(CONDITION_PARAM_COUNT, 10);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 10);

			count /= 20;
		}
		else if (condition->getType() == CONDITION_DEATH) {
			condition->setParam(CONDITION_PARAM_COUNT, 10);
			condition->setParam(CONDITION_PARAM_MAX_COUNT, 10);
...
int LuaScriptInterface::luaMonsterTypeCanHitBack(lua_State* L)
{
	// monsterType:isPassive()
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		pushBoolean(L, monsterType->info.canHitBack);
