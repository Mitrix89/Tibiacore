...
bool Monster::selectTarget(Creature* creature)
{
	...
	if (canHitBack() && mType->info.targetDistance <= 1 && mType->info.isHostile && !hasBeenAttacked(creature->getID())) {
		// tipo passiva, não corre e não ataca entra me modo dacing.
		return false;
	}
...
{
	if (type == CONDITION_FIRE || type == CONDITION_ENERGY || type == CONDITION_POISON || type == CONDITION_ICE || type == CONDITION_HOLY || type == CONDITION_DEATH || type == CONDITION_AGGRESSIVE) {
		updateMapCache();
	}
