...
ConditionType_t Combat::DamageToConditionType(CombatType_t type)
{
	switch (type) {
		...
		case COMBAT_CUSTOM_PHYSICAL:
			...
			// Odwrotna konwersja dla Ice
		case COMBAT_ICEDAMAGE:
			return CONDITION_ICE;

			// Odwrotna konwersja dla Death
		case COMBAT_DEATHDAMAGE:
			return CONDITION_DEATH;

			// Odwrotna konwersja dla San
		case COMBAT_HOLYDAMAGE:
			return CONDITION_HOLY;
...
bool Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, CombatDamage* data)
{
	...
	if (target->getMonster() && !target->isSummon()) {
		// Revidar attack
		Monster* monster = target->getMonster();
		monster->hitBackNoHostil(caster);
	}
...
int32_t Combat::getTotalDamage(int32_t attackSkill, int32_t attackValue, fightMode_t fightMode)
{
	...
	int32_t formula = (5.5 * (attackSkill) + 60) * damage;
				...
				if (player->getAddAttackSkill()) {
					...
					case BLOCK_NONE: {
						if (player->getVocationId() == 4 || player->getVocationId() == 8) {
							player->addSkillAdvance(static_cast<skills_t>(skill), 2);
						} else {
							player->addSkillAdvance(static_cast<skills_t>(skill), 1);
						}
						break;
		...
		if (weapon) {
			...
			if (weapon->getWeaponSpecialEffect() == 3 && weapon->getWeaponType() != WEAPON_DISTANCE
				...
				if (hit) {
					...
					bool weapon_holy = DamageToConditionType(weapon->getCombatType()) == CONDITION_HOLY;
					bool weapon_ice = DamageToConditionType(weapon->getCombatType()) == CONDITION_ICE;
					bool weapon_death = DamageToConditionType(weapon->getCombatType()) == CONDITION_DEATH;
					...
					else if (weapon_holy == true && check_condition == false) {
						Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5~10 normal arrow
						Damage->setParam(CONDITION_PARAM_MAXVALUE, normal_random(5, (weapon_skill / 5)));
						Damage->setParam(CONDITION_PARAM_HIT_DAMAGE, normal_random(5, (weapon_skill / 5)));
						g_game.addMagicEffect(targetPos, weapon->getEffectItem());
					...
					else if (weapon_ice == true && check_condition == false) {
						Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5~10 normal arrow
						Damage->setParam(CONDITION_PARAM_MAXVALUE, normal_random(5, (weapon_skill / 5)));
						Damage->setParam(CONDITION_PARAM_HIT_DAMAGE, normal_random(5, (weapon_skill / 5)));
						g_game.addMagicEffect(targetPos, weapon->getEffectItem());
					...
					else if (weapon_death == true && check_condition == false) {
						Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5~10 normal arrow
						Damage->setParam(CONDITION_PARAM_MAXVALUE, normal_random(5, (weapon_skill / 5)));
						Damage->setParam(CONDITION_PARAM_HIT_DAMAGE, normal_random(5, (weapon_skill / 5)));
						g_game.addMagicEffect(targetPos, weapon->getEffectItem());
...
bool Combat::rangeAttack(Creature* attacker, Creature* target, fightMode_t fightMode)
{
	...
	if (Player* player = attacker->getPlayer()) {
		...
		// spear and other drop on ground, true to drop
		if (g_config.getBoolean(ConfigManager::DROP_SPEAR_FREE)) {
			if (!player->isPremium()) {
				moveWeapon = true;
			}
		}
		if (g_config.getBoolean(ConfigManager::DROP_SPEAR_PREMIUM)) {
			if (player->isPremium()) {
				moveWeapon = true;
			}
		}
	...
	if (weapon->getWeaponType() == WEAPON_DISTANCE) {
		...
		if (weapon && weapon->getAmmoType() == AMMO_NONE) {
			hitChance = 45; // <75 throwables and such
		...
		if (rand() % distance <= static_cast<int32_t>(skillValue)) {
		...
		if (specialEffect == 1) {
			...
			bool ammo_ice = DamageToConditionType(ammunition->getCombatType()) == CONDITION_ICE;
			bool ammo_death = DamageToConditionType(ammunition->getCombatType()) == CONDITION_DEATH;
			bool ammo_holy = DamageToConditionType(ammunition->getCombatType()) == CONDITION_HOLY;
			...
			if (weapon_ice == true && ammo_ice == true) {
				if (check_condition == false) {
					Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 7)); // full hit 5<40 (100~175) bow/arrow energy
					Damage->setParam(CONDITION_PARAM_MAXVALUE, w_a);
				}
			...
			if (weapon_holy == true && ammo_holy == true) {
				if (check_condition == false) {
					Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 7)); // full hit 5<40 (100~175) bow/arrow energy
					Damage->setParam(CONDITION_PARAM_MAXVALUE, w_a);
				}
			...
			if (weapon_death == true && ammo_death == true) {
				if (check_condition == false) {
					Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 7)); // full hit 5<40 (100~175) bow/arrow energy
					Damage->setParam(CONDITION_PARAM_MAXVALUE, w_a);
				}
			...
			if (weapon_ice == true && ammo_ice == false && check_condition == false) {
				Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5<25 (60~90) normal arrow
				Damage->setParam(CONDITION_PARAM_MAXVALUE, (w_a - ammunition->getAttack()) + 15);
				g_game.addMagicEffect(targetPos, weapon->getEffectItem());
			}
			if (weapon_holy == true && ammo_holy == false && check_condition == false) {
				Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5<25 (60~90) normal arrow
				Damage->setParam(CONDITION_PARAM_MAXVALUE, (w_a - ammunition->getAttack()) + 15);
				g_game.addMagicEffect(targetPos, weapon->getEffectItem());
			}
			if (weapon_death == true && ammo_death == false && check_condition == false) {
				Damage->setParam(CONDITION_PARAM_CYCLE, normal_random(3, 8)); // hit 5<25 (60~90) normal arrow
				Damage->setParam(CONDITION_PARAM_MAXVALUE, (w_a - ammunition->getAttack()) + 15);
				g_game.addMagicEffect(targetPos, weapon->getEffectItem());
			}
		...
		impact.actor = attacker;
		impact.damage.type = combatParams.combatType;
		impact.damage.value = -Combat::computeDamage(attacker, weapon->getAttackStrength(), weapon->getAttackVariation());
		circleShapeSpell(attacker, target->getPosition(), 0xFF, weapon->getMissileType(), weapon->getRadius(), &impact, weapon->getEffectItem());
	}
		...
		if (weapon) {
			...
			case WEAPON_DISTANCE: {
				...
				if (weapon->getAmmoType() != AMMO_NONE) {
					...
					} else if (ammunition->getWeaponType() == weapon->getWeaponType()) {
						skill = SKILL_FIST;
						attackValue -= attackValue;
						break;

					} else if (ammunition->getAmmoType() != weapon->getAmmoType()) {
						skill = SKILL_FIST;
						attackValue -= attackValue;
						break;
