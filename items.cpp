			...
			while (true) {
				while (true) {
					...
					if (identifier == "waypoints") {
						...
						items[id].radiusItem = script.readNumber();
					} else if (identifier == "effect") {
						items[id].effectItem = script.readNumber();
					...
					} else if (identifier == "luck") {
						items[id].luck = script.readNumber();
					} else if (identifier == "luckgold") {
						items[id].luckGold = script.readNumber();
					} else if (identifier == "luckjewel") {
						items[id].luckJewel = script.readNumber();
					} else if (identifier == "luckrune") {
						items[id].luckRune = script.readNumber();
					} else if (identifier == "luckcharge") {
						items[id].luckCharge = script.readNumber();
						...
						items[id].getAbilities().absorbPercent[combatTypeToIndex(COMBAT_ICEDAMAGE)] += percent;
						items[id].getAbilities().absorbPercent[combatTypeToIndex(COMBAT_HOLYDAMAGE)] += percent;
						items[id].getAbilities().absorbPercent[combatTypeToIndex(COMBAT_DEATHDAMAGE)] += percent;
						...
						} else if (identifier == "custompoison") {
							conditionDamage = new ConditionDamage(CONDITIONID_COMBAT, CONDITION_POISON);
							conditionDamage->setParam(CONDITION_PARAM_DELAYED, true);
							combatType = COMBAT_EARTHDAMAGE;
							items[id].combatType = combatType;
							items[id].conditionDamage.reset(conditionDamage);
						} else if (identifier == "customenergy") {
