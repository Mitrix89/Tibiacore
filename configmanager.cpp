...
bool ConfigManager::load()
{
	...
	boolean[EXPERIENCE_FROM_PLAYERS] = getGlobalBoolean(L, "experienceByKillingPlayers", true);
	...
	boolean[STACK_CUMULATIVES] = getGlobalBoolean(L, "autoStackCumulatives", true);
	...
	boolean[MAKE_RUNE_BACKPACK_FREE] = getGlobalBoolean(L, "MakeRuneBackpackFree", true);
	boolean[MAKE_RUNE_BACKPACK_PREMIUM] = getGlobalBoolean(L, "MakeRuneBackpackPremium", true);
	boolean[ENABLE_LIVE_CASTING] = getGlobalBoolean(L, "enableLiveCasting", false);
	boolean[SAVE_PLAYERS_LOGS] = getGlobalBoolean(L, "SavePlayersLogs", true);
	...
	integer[NEWBIE_TOWN] = getGlobalNumber(L, "newbieTownId", 11);
